...
def filter_reassemble_complexes(lr_res,
    ...
    _complex_cols
        method-relevant columns TODO change to _method_cols
    expr_prop
    ...
    if duplicate_mask.any():
        ...
        if not lr_res[duplicate_mask].groupby(_key_cols)[_complex_cols].transform(lambda x: x.duplicated(keep=False)).all().all():
