...
class SpatialInflow:
    ...
    def __call__(
        ...
        **kwargs
        ...
        assert np.all(ct.sum(axis=1) == 1)  # check proper one-hot encoding

        # Compute global stats (proportions) for all features in adata
        xy_stats = (
            pd.DataFrame({"props": _get_props(adata.X)}, index=adata.var_names)
            .reset_index()
        ...
        x_mat = adata[:, xy_stats[self.x_name]].X  # ligand expression
        ...
        xy_mat = wls * ri  # shape (n_cells, n_LRs * n_ct)
